...
func (c *cmdQuery) Run(cmd *cobra.Command, args []string) error {
	...
	if c.global.flagProject != "" {
		return fmt.Errorf("%s", i18n.G("--project cannot be used with the query command"))
	}
	...
	if !strings.HasPrefix(path, "/") {
		return fmt.Errorf("%s", i18n.G("Query path must start with /"))
	}
