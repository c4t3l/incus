...
var devIncusAPIHandler = devIncusHandler{"/1.0", func(d *Daemon, c instance.Instance, w http.ResponseWriter, r *http.Request) response.Response {
	...
	if r.Method == "GET" {
		...
		if err != nil {
			return response.DevIncusErrorResponse(api.StatusErrorf("%d", http.StatusBadRequest, err.Error()), c.Type() == instancetype.VM)
		}
		...
		if err != nil {
			return response.DevIncusErrorResponse(api.StatusErrorf("%d", http.StatusInternalServerError, err.Error()), c.Type() == instancetype.VM)
		}
	...
	return response.DevIncusErrorResponse(api.StatusErrorf("%d", http.StatusMethodNotAllowed, fmt.Sprintf("method %q not allowed", r.Method)), c.Type() == instancetype.VM)
}}
