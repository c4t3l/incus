...
var devIncusAPIHandler = devIncusHandler{"/1.0", func(d *Daemon, c instance.Instance, w http.ResponseWriter, r *http.Request) response.Response {
	...
	if r.Method == "GET" {
		...
		if err != nil {
			return response.DevIncusErrorResponse(api.StatusErrorf("%v", http.StatusBadRequest, err.Error()), c.Type() == instancetype.VM)
		}
		...
		if err != nil {
			return response.DevIncusErrorResponse(api.StatusErrorf("%v", http.StatusInternalServerError, err.Error()), c.Type() == instancetype.VM)
		}
	...
	return response.DevIncusErrorResponse(api.StatusErrorf("%v", http.StatusMethodNotAllowed, fmt.Sprintf("method %q not allowed", r.Method)), c.Type() == instancetype.VM)
}}
